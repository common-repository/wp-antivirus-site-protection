    ...
    */
    ...
    if (isset($avp_license_info['membership']))	
    {
		if ($avp_alert_main > 0 || $avp_alert_heuristic > 0 || $avp_license_info['blacklist']['google'] != 'ok' )
		{
            ...
            if ($avp_license_info['membership'] == 'trial')
            {
                $avp_eachpage_alert_txt .= '<p style="color:#DD3D36;font-size:15px;">Limited offer! For all new sites use coupon <b>CLEAN45OFF</b> and <a href="https://www.siteguarding.com/en/buy-service/website-malware-and-blacklist-removal-service?pgid=WP45" target="_blank">Get 45% OFF for Malware Removal service</a></p>';
            }
		} 
...
public static function PrintIconMessage($data)
{
	...
	public static function page_PreScan($params)
	{
        ...
        {
            $data['hide_alert_till'] = date("Y-m-d", mktime(0, 0, 0, date("m")  , date("d")+14, date("Y")));
            plgwpavp_SetExtraParams($data);
        }
    ...
    if (count($params['reports']) > 0) 
    {
        ...
        else if ($params['last_scan_files_counters']['heuristic'] > 0) 
        {
            $message_data = array(
                ...
                'message' => 'We have detected virus / unsafe files on your website.<br><br>LIMITED OFFER!<br> For all new sites use coupon <b>CLEAN45OFF</b> and <a href="https://www.siteguarding.com/en/buy-service/website-malware-and-blacklist-removal-service?pgid=WP45" target="_blank">Get 45% off Malware Removal service</a>',
                'button_text' => $button_text,
                'button_url' => $link_clean,
                'help_text' => $extra_clean_text
            );
